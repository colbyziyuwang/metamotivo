# Hyper parameter tuning for lagrange
...
METHOD = "gradient" # gradient_descent, baseline, bisection
...
if __name__ == "__main__":
    ...
    output_file = f"lagrange_output_{body_part}_{kind} ({METHOD})_hyp_tune.txt"
    ...
    with open(output_file, "w") as f:
        ...
        for hyper_param_combination in itertools.product(*hyper_params.values()):
            lambda_t, eta, lr, n_steps = hyper_param_combination
            
            # need to record the values of hyper parameters
            hyperparam_str = (
                f"📌 Hyperparameters:\n"
                f"• lambda_init: {lambda_t:.4f}\n"
                f"• eta:         {eta}\n"
                f"• lr:          {lr}\n"
                f"• steps:       {n_steps}\n"
            )
            ...
            for seed in range(5):
                ...
                lambda_t = torch.tensor(float(lambda_t), dtype=torch.float32, device=z.device,
                ...
                for step in range(n_steps):
                    ...
                    if (METHOD == "gradient_descent"):
                        ...
                        sign = -1 * torch.sign(Q.mean() - eta)        # -1 if Q>η, +1 if Q<η
                        # heuristic: move towards the sign to push Q toward η
