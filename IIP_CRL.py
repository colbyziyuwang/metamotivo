# This script combines Inverse Inverse Planning with Constrained RL
...
if __name__ == "__main__":
    ...
    output_file = f"IIP_CRL_output_{body_part}_{kind} ({METHOD}).txt"
    ...
    with open(output_file, "w") as f:
        for task in STANDARD_TASKS:
            ...
            for seed in range(5):
                ...
                # Note we will modify the reward term
                back_embed = model.backward_map(observation_c_z_tensor)
                IIP_r = (back_embed - z_star).pow(2).sum(dim=-1, keepdim=True).sqrt()
                w = 1.0 # weight for IIP reward
                reward_term += w * IIP_r
                c_z = model.reward_inference(observation_c_z_tensor, reward_term)

                eta = -300
                lr = 0.0001 
                obs_torch = torch.tensor(observation.reshape(1, -1), dtype=torch.float32, device=z_star.device)
                ...
                lambda_t = torch.tensor(lambda_t, dtype=torch.float32, device=z_star.device,
                                        requires_grad=requires_grad)
                                    
                lambda_min_t = torch.tensor(lambda_min_t, dtype=torch.float32, device=z_star.device,
                                        requires_grad=False)
                lambda_max_t = torch.tensor(lambda_max_t, dtype=torch.float32, device=z_star.device,
                                        requires_grad=False)
                ...
                for step in range(100):
                    ...
                    # --- build Z_{λ,c} ---
                    Z_lambda_c = z_star - lambda_t * c_z          # this depends on λ
                    ...
                    # Note we will now modify how Q is computed (note Q is minus cost)
                    
                    # Begin rollout and collect history of (s, r)
                    n_steps = 10
                    hist = []

                    observation, _ = env.reset()
                    for _ in range(n_steps):
                        obs = torch.tensor(observation.reshape(1, -1), dtype=torch.float32, device=z_star.device)
                        action = rew_model.act(obs, Z_lambda_c).ravel()
                        observation, reward, terminated, truncated, info = env.step(action)
                        hist.append((obs, reward))
                    
                    # Sample a tuple of (s, r) from history
                ...
                frames = [env.render()]
                while not done:
                    Z_lambda_c = z_star - lambda_t.item() * c_z
                    obs = torch.tensor(observation.reshape(1, -1), dtype=torch.float32, device=z_star.device)
                    action = rew_model.act(obs, Z_lambda_c).ravel()
                    observation, reward, terminated, truncated, info = env.step(action)
                    total_reward += reward
                    ...
                    if n_steps == 0:
                        action = torch.tensor(action, dtype=torch.float32, device=z_star.device)
                ...
                video_filename = f"{task.replace('/', '_')}_seed{seed}_{range_str}_{dims_str}_eta_{eta}_{body_part}_{kind}_IIPCRL.mp4"
